      ...
      {phonemeTags.map((phonemeTag, index) => (
        <li key={index}>
          <button
            ...
            style={
              phonemeTag === selectedPhonemeTag ? { color: phonemeColor } : null
            }
...
export default class TopNav extends React.Component {
  ...
  updatePhonemeTag(selectedPhonemeTag) {
    ...
    const phonemeColorType =
      selectedPhonemeTag === 'All'
        ? (phonemeColor = null)
        : selectedPhonemeTag === 'AH'
        ? (phonemeColor = 'magenta')
        : selectedPhonemeTag === 'EH'
        ? (phonemeColor = 'red')
        : selectedPhonemeTag === 'EEH'
        ? (phonemeColor = 'green')
        : selectedPhonemeTag === 'OH'
        ? (phonemeColor = 'gold')
        : selectedPhonemeTag === 'OOH'
        ? (phonemeColor = 'blue')
        : null;
  ...
  render() {
    const { selectedPhonemeTag, phonemeColor } = this.state;

    return (
      <ThemeConsumer>
        {({ theme, toggleTheme }) => (
          <div className="top-nav">
            <NavLink to="/batches" exact className="nav-link">
              Posts
            </NavLink>
            <NavLink to="/create-phrase" exact className="nav-link">
              Create Phrase
            </NavLink>
            <PhonemeNav
